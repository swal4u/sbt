...
RUN apt-get update && apt-get install pass pinentry-tty git gnupg2 zip unzip gzip curl clang llvm gcc openjdk-21-source -y && \
...
COPY --from=builder /usr/lib/jvm/java-21-openjdk-amd64/lib/src.zip /opt/java/openjdk/
RUN ldconfig
WORKDIR /app
